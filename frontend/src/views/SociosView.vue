...
const handleSaveSocio = (savedSocio) => {
  ...
  if (currentAction.value === 'search' && searchResult.value && (searchResult.value.id === savedSocio.id || searchResult.value.id === selectedSocioId.value)) {
...
<template>
  <div class="min-h-screen bg-slate-50 font-sans text-slate-800">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="currentAction !== 'none'"
        ...
        <div v-if="currentAction === 'add'" class="flex flex-col items-center justify-center h-full py-12 text-center">
          ...
          <p class="text-slate-500 mt-2 max-w-md">El formulario se ha abierto en una ventana emergente. Si la cerró,
            puede volver a abrirla con el botón de abajo.</p>
    ...
    <!-- Confirmation Modal -->
    <ConfirmModal :is-open="isConfirmDeleteOpen" title="Confirmar Baja de Socio"
      :message="socioToDelete ? `¿Está seguro que desea dar de baja a ${socioToDelete.nombre} ${socioToDelete.apellido}? Esta acción no se puede deshacer.` : ''"
