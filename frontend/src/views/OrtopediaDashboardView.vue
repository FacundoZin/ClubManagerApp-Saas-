  ...
  <div class="min-h-screen bg-slate-50 font-sans text-slate-800">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
       ...
       <div class="mb-6">
        <nav class="flex mb-2" aria-label="Breadcrumb">
          <ol class="inline-flex items-center space-x-1 md:space-x-3">
             <li class="inline-flex items-center">
              <a href="#" @click.prevent="goHome" class="inline-flex items-center text-sm font-medium text-slate-500 hover:text-teal-600">
        ...
        <div class="flex justify-between items-end">
             <div>
                ...
                <p class="text-slate-500 mt-1">Gestión de artículos ortopédicos y seguimiento de alquileres.</p>
             </div>
             <div>
      ...
      <div class="border-b border-slate-200 mb-6">
          <nav class="-mb-px flex space-x-8" aria-label="Tabs">
              <button @click="activeTab = 'articulos'"
                  :class="[activeTab === 'articulos' ? 'border-teal-500 text-teal-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300', 'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors']">
                  Gestión de Artículos
              </button>
              <button @click="activeTab = 'alquileres'"
                  :class="[activeTab === 'alquileres' ? 'border-teal-500 text-teal-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300', 'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors']">
      ...
      <div v-if="activeTab === 'alquileres'">
          <!-- Actions / Search Bar -->
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
             <div class="w-full sm:w-auto flex-1 max-w-lg">
                 <label for="search-dni" class="block text-xs font-medium text-slate-500 mb-1">Buscar por DNI Socio</label>
                 <div class="relative rounded-md shadow-sm">
                    ...
                    <button v-if="searchDni" @click="searchDni = ''; loadAlquileresActivos()" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600">
         ...
         <div v-if="!loadingAlquileres">
             ...
             <!-- List -->
             <div v-if="(isSearching ? searchResultAlquileres : alquileres).length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 <AlquilerCard 
    ...
    <!-- Modals -->
    <ArticuloFormModal :is-open="isArticuloModalOpen" @close="isArticuloModalOpen = false" @save="handleCreateArticulo" />
    <UpdatePrecioModal :is-open="isUpdatePrecioModalOpen" :articulo="selectedArticulo" @close="isUpdatePrecioModalOpen = false" @save="handleUpdatePrecio" />
