            ...
            <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="h-32 bg-gradient-to-br from-blue-600 to-indigo-700 relative">
                            <div class="absolute -bottom-10 left-1/2 -translate-x-1/2">
                                <div
                                    ...
                                    <span class="text-3xl font-black">{{ socio.nombre[0] }}{{ socio.apellido[0]
                                        }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="pt-14 pb-8 px-6 text-center">
                            <h2 class="text-2xl font-black text-slate-900">{{ socio.nombre }} {{ socio.apellido }}</h2>
                            <p class="text-slate-500 font-bold uppercase tracking-wider text-xs mt-1">Socio #{{ socio.id
                                }}</p>

                            <div class="mt-6 inline-flex items-center px-4 py-1.5 rounded-full text-sm font-black ring-4 ring-white shadow-sm"
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 space-y-6">
                        <h3 class="text-sm font-black text-slate-400 uppercase tracking-widest">Información de contacto
                        </h3>
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-slate-500 text-sm font-medium">{{ socio.lote ? 'Lote: ' + socio.lote : ''
                                    }} {{ socio.localidad }}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    <tr v-for="cuota in socio.historialCuotas" :key="cuota.id"
                                        class="hover:bg-slate-50/80 transition-colors">
                                        <td class="px-8 py-5 text-sm font-bold text-slate-700">{{
                                            formatDate(cuota.fechaPago) }}</td>
                                        <td class="px-8 py-5 text-sm font-black text-slate-900">${{
                                            cuota.importe.toLocaleString('es-AR') }}</td>
