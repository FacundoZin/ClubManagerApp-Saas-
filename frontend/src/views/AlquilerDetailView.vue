...
import ConfirmModal from '../Components/ConfirmModal.vue' // Note casing: using Components as fixed previously
...
const goBack = () => router.push('/ortopedia')
</script>
...
<template>
  <div class="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12">
       ...
       <div class="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-30">
           <div class="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
               <div class="flex items-center gap-3">
                   <button @click="goBack" class="p-2 -ml-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100 transition-colors">
                       <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                           <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                       </svg>
                   </button>
                   <h1 class="text-xl font-bold text-slate-900">Detalle de Alquiler #{{ alquilerId }}</h1>
               </div>
                <div v-if="alquiler">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium"
                      :class="alquiler.estaAlDia ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'">
       ...
       <div v-else-if="error" class="max-w-3xl mx-auto mt-12 p-6 bg-red-50 text-red-700 rounded-lg text-center">
           {{ error }}
       </div>
       ...
       <div v-else class="max-w-5xl mx-auto px-4 sm:px-6 mt-8 space-y-6">
           ...
           <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                   <h2 class="text-sm font-medium text-slate-500 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Información del Socio</h2>
                   <div class="space-y-3">
                        <div class="flex justify-between">
                            <span class="text-slate-500">Nombre:</span>
                            <span class="font-medium text-slate-900">{{ alquiler.nombreSocio }} {{ alquiler.apellidoSocio }}</span>
                        </div>
                        <div class="flex justify-between">
                         ...
                         <div class="flex justify-between" v-if="alquiler.direccionSocio">
                            ...
                            <span class="font-medium text-slate-900 text-right">{{ alquiler.direccionSocio }} <br><span class="text-xs text-slate-400">{{ alquiler.localidadSocio }}</span></span>
                        </div>
                   </div>
               </div>
               ...
               <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col">
                   <h2 class="text-sm font-medium text-slate-500 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Estado y Acciones</h2>
                   ...
                   <div class="mt-6 flex flex-col gap-3">
                       <button @click="handleRegisterPayment"
           ...
           <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
               <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    ...
                    <button @click="isAddItemModalOpen = true" class="text-sm font-medium text-teal-600 hover:text-teal-800 flex items-center">
               ...
               <table class="min-w-full divide-y divide-slate-200">
                   <thead class="bg-white">
                       <tr>
                           <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Artículo</th>
                           <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Precio Unit.</th>
                   ...
                   <tbody class="divide-y divide-slate-200 bg-white">
                       <tr v-for="item in alquiler.items" :key="item.id">
                           <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{{ item.nombreArticulo }}</td>
                           <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">${{ item.precioAlquiler }}</td>
                           <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                               <div class="flex items-center space-x-2">
                                   <button @click="handleUpdateQuantity(item, item.cantidad - 1)" class="p-1 text-slate-400 hover:text-slate-600 border rounded">-</button>
                                   <span>{{ item.cantidad }}</span>
                                   <button @click="handleUpdateQuantity(item, item.cantidad + 1)" class="p-1 text-slate-400 hover:text-slate-600 border rounded">+</button>
                               </div>
                           </td>
                           <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">${{ item.subtotal }}</td>
                           <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                               <button @click="handleRemoveItem(item.id)" class="text-rose-600 hover:text-rose-900 font-medium">Eliminar</button>
                           </td>
                       </tr>
                       <tr v-if="!alquiler.items || alquiler.items.length === 0">
                           <td colspan="5" class="px-6 py-8 text-center text-slate-400 italic">No hay artículos en este alquiler.</td>
                       </tr>
                   </tbody>
               </table>
           </div>
           ...
           <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-12">
               ...
               <div class="p-6 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4" v-if="alquiler.historialDePagos && alquiler.historialDePagos.length > 0">
                    <div v-for="pago in alquiler.historialDePagos" :key="pago.id" 
                         class="bg-emerald-50 border border-emerald-100 rounded-lg p-3 text-center">
                        <div class="text-xs text-emerald-600 uppercase font-semibold">{{ pago.mes }}/{{ pago.anio }}</div>
      ...
      <!-- Modals -->
      <AddItemModal :is-open="isAddItemModalOpen" :alquiler-id="parseInt(alquilerId)" @close="isAddItemModalOpen = false" @save="handleAddItem" />
      
      <ConfirmModal :is-open="isConfirmFinalizeOpen" 
      ...
      <div v-if="toast.show" class="fixed bottom-5 right-5 z-50 flex w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-2xl border border-slate-200 pointer-events-auto ring-1 ring-black ring-opacity-5">
        <div class="flex items-center justify-center w-12" :class="{ 'bg-teal-500': toast.type === 'success', 'bg-red-500': toast.type === 'error' }">
        ...
        <div class="px-4 py-3">
          <span class="font-semibold" :class="{ 'text-teal-500': toast.type === 'success', 'text-red-500': toast.type === 'error' }">
