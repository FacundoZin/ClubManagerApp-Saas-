...
<template>
  <div class="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
          <div class="flex items-center gap-3 cursor-pointer" @click="goHome">
            <div class="w-9 h-9 bg-teal-600 rounded-lg flex items-center justify-center shadow-md text-white">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-6">
          ...
          <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
              <div class="flex items-center">
                ...
                <span class="ml-1 text-sm font-medium text-slate-700 md:ml-2">Alquiler Artículos</span>
              </div>
            </li>
          </ol>
        </nav>
        <div class="flex justify-between items-end">
          ...
          <div>
            <button v-if="activeTab === 'articulos'" @click="isArticuloModalOpen = true"
      ...
      <div class="border-b border-slate-200 mb-6">
        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
          <button @click="activeTab = 'articulos'" :class="[
            activeTab === 'articulos'
          ...
          <button @click="activeTab = 'gestionar-alquileres'" :class="[
          ...
          <button @click="activeTab = 'nuevo-alquiler'" :class="[
      ...
      <div v-if="activeTab === 'articulos'">
        ...
        <div v-else-if="articulos.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <ArticuloCard v-for="art in articulos" :key="art.id" :articulo="art" @update-price="openUpdatePrecio" />
        </div>
      ...
      <div v-if="activeTab === 'gestionar-alquileres'">
        ...
        <div
          ...
          <div class="w-full sm:w-auto flex-1 max-w-lg">
            <label for="search-dni" class="block text-xs font-medium text-slate-500 mb-1">Buscar cualquier alquiler por
              el socio</label>
            <div class="relative rounded-md shadow-sm">
              ...
              <button v-if="searchDni" @click="loadAlquileresActivos()"
        ...
        <!-- Error -->
        <div v-if="searchError && !loadingAlquileres" class="bg-red-50 text-red-700 p-4 rounded-lg mb-6 text-center">
          {{ searchError }}
        </div>
        ...
        <div v-if="!loadingAlquileres">
          ...
          <!-- List -->
          <div v-if="(isSearching ? searchResultAlquileres : alquileres).length > 0"
            class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <AlquilerCard v-for="alq in isSearching ? searchResultAlquileres : alquileres" :key="alq.id" :alquiler="alq"
              @view-detail="goToAlquilerDetail" />
          </div>

          <!-- Empty State -->
          <div v-else class="text-center py-12 bg-white rounded-lg border border-slate-200 border-dashed">
      ...
      <div v-if="activeTab === 'nuevo-alquiler'">
        ...
        <div v-if="!foundSocio" class="max-w-2xl mx-auto">
          <div class="bg-white p-8 rounded-xl border border-slate-200 shadow-sm text-center">
            <div class="w-16 h-16 bg-teal-50 text-teal-600 rounded-full flex items-center justify-center mx-auto mb-6">
            ...
            <div class="flex gap-2 max-w-md mx-auto">
              <div class="relative flex-1">
                <input type="text" v-model="searchSocioDni" @keyup.enter="handleSearchSocio"
              ...
              <button @click="handleSearchSocio" :disabled="searchingSocio || !searchSocioDni"
            ...
            <div v-if="searchError" class="mt-4 text-sm text-red-600 bg-red-50 p-3 rounded-lg border border-red-100">
              {{ searchError }}
            </div>
          </div>
        </div>
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <div class="flex justify-between items-start mb-4">
                ...
                <button @click="resetNuevoAlquiler" class="text-xs text-slate-500 hover:text-red-500 underline">
                  Cambiar Socio
                </button>
              </div>
              <div class="flex items-center gap-6 bg-slate-50 p-6 rounded-xl border border-slate-100">
                ...
                <div class="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div class="sm:col-span-1">
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Socio</p>
                  ...
                  <div>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Contacto</p>
                    <p class="text-base font-medium text-slate-700">{{ foundSocio.telefono || 'N/A' }}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              ...
              <div v-if="articulos.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div v-for="art in articulos" :key="art.id"
                  ...
                  <div>
                    ...
                    <p class="text-xs text-teal-600 font-bold">${{ art.precioAlquiler.toLocaleString() }} / mes</p>
                  </div>
                  <button @click="addArticuloToAlquiler(art)"
          ...
          <div class="space-y-6">
            <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm sticky top-24">
              ...
              <div class="space-y-4 mb-6">
                <div v-if="newAlquilerItems.length === 0"
                ...
                <div v-else v-for="(item, index) in newAlquilerItems" :key="item.articuloId"
                  ...
                  <div class="flex-1">
                    ...
                    <div class="flex items-center gap-2 mt-1">
                      <label class="text-[10px] text-slate-400 uppercase tracking-wider">Cant:</label>
                      <div class="flex items-center border border-slate-200 rounded-md overflow-hidden bg-slate-50">
                        ...
                        <span class="px-2 text-xs font-bold text-slate-700 min-w-[24px] text-center">
                          {{ item.cantidad }}
                        </span>
                  ...
                  <div class="text-right">
                    <p class="text-sm font-bold text-teal-600">${{ (item.precio * item.cantidad).toLocaleString() }}</p>
                    <button @click="removeArticuloFromAlquiler(index)"
              ...
              <div class="mb-6">
                ...
                <textarea v-model="observaciones" rows="3"
                  ...
                  placeholder="Ej: El artículo se entrega con detalles de uso normal..."></textarea>
              </div>
              ...
              <div class="flex justify-between items-center py-4 border-t border-slate-100 mb-6">
                ...
                <span class="text-2xl font-black text-slate-900">
                  ${{newAlquilerItems.reduce((acc, i) => acc + i.precio * i.cantidad, 0).toLocaleString()}}
                </span>
              </div>

              <!-- Action -->
              <button @click="handleRegisterAlquiler" :disabled="isRegisteringAlquiler || newAlquilerItems.length === 0"
    ...
    <!-- Modals -->
    <ArticuloFormModal :is-open="isArticuloModalOpen" @close="isArticuloModalOpen = false"
      @save="handleCreateArticulo" />
    <UpdatePrecioModal :is-open="isUpdatePrecioModalOpen" :articulo="selectedArticulo"
      @close="isUpdatePrecioModalOpen = false" @save="handleUpdatePrecio" />
