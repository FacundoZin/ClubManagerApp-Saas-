...
import SocioRentStatusCard from '../../components/ModuloAlquilerArticulos/Alquileres/SocioRentStatusCard.vue'
...
const rentCheckStatus = ref(null) // Nuevo estado para el resultado del chequeo
...
const handleSearchSocio = async () => {
  ...
  searchError.value = ''
  try {
    // CAMBIO: Ahora consultamos primero el estado del socio en alquileres
    // Esto retorna un StatusAlquileresSociosDto
...
const startNewAlquiler = async () => {
  // Si el usuario decide crear uno nuevo, necesitamos el objeto Socio completo
  // El DTO de status ya trae datos básicos, pero si necesitamos el objeto completo de socio:
  // Option A: Usar los datos del DTO (Nombre, Apellido, DNI implicito) y construir un objeto parcial.
  // Option B: Hacer fetch del socio completo aquí.
  // Vamos con Option B para asegurar tener todos los datos consistentes (como ID real si no viniera en el Status DTO, aunque sí viene).

  // StatusAlquileresSociosDto trae IdSocio.
  // Vamos a obtener el socio completo para llenar 'foundSocio' que usa la UI actual.
  searchingSocio.value = true
  try {
    // Podemos usar el servicio getById si lo tenemos, o getByDni si preferimos.
    // Como ya tenemos el DNI en searchSocioDni, usamos getByDni para mantener consistencia,
    // o podríamos implementar getById en SociosService si no existe.
    // Revisando services, SociosService.getByDni existe.
    const socio = await SociosService.getByDni(searchSocioDni.value)
    foundSocio.value = socio
    rentCheckStatus.value = null // Limpiamos el status card para mostrar el formulario
  } catch (e) {
...
const navigateToAlquiler = (idAlquiler) => {
  goToAlquilerDetail(idAlquiler)
}

const cancelSearch = () => {
  rentCheckStatus.value = null
  searchSocioDni.value = ''
  // Focus back on input ideally
}
  ...
  <div class="min-h-screen bg-slate-50 font-sans text-slate-800">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="activeTab === 'nuevo-alquiler'">
        <!-- Paso 1: Buscar Socio -->
        <div v-if="!foundSocio && !rentCheckStatus" class="max-w-2xl mx-auto">
