...
<template>
  <div class="bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden flex flex-col h-full">
    <div class="p-5 flex-grow">
      ...
      <div class="mt-4 space-y-2">
        <div class="flex items-center text-sm text-slate-600">
           <span class="font-bold mr-2">Deuda:</span> ${{ socio.deuda || '0.00' }}
        </div>
        <div class="flex items-center text-sm text-slate-600">
    ...
    <div class="bg-slate-50 px-5 py-3 border-t border-slate-100 flex flex-col gap-2">
      <button 
        @click="$emit('pay', socio)"
        class="w-full inline-flex justify-center items-center rounded-lg bg-green-600 px-3 py-2 text-sm font-semibold text-white hover:bg-green-500 shadow-sm transition-all"
      >
        Registrar pago
      </button>
