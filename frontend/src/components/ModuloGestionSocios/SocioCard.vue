  ...
  <div
      ...
      <div class="flex justify-between items-start">
        ...
        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold"
          ...
          <span class="w-1.5 h-1.5 rounded-full mr-1.5"
            :class="socio.adeudaCuotas ? 'bg-red-500' : 'bg-green-500'"></span>
    ...
    <div class="bg-slate-50 px-5 py-3.5 border-t border-slate-100 flex justify-end gap-2">
      <button @click="$emit('view', socio)"
        class="text-xs font-bold text-slate-600 hover:text-blue-600 px-2.5 py-1.5 rounded-lg hover:bg-slate-200 transition-colors inline-flex items-center gap-1.5">
      ...
      <button @click="$emit('edit', socio)"
        class="text-xs font-bold text-blue-600 hover:text-blue-800 px-2.5 py-1.5 rounded-lg hover:bg-blue-50 transition-colors inline-flex items-center gap-1.5">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <button @click="$emit('delete', socio)"
        class="text-xs font-bold text-red-600 hover:text-red-800 px-2.5 py-1.5 rounded-lg hover:bg-red-50 transition-colors inline-flex items-center gap-1.5">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
