...
<template>
  <div
    class="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden"
  >
    <div class="p-5">
      <div class="flex justify-between items-start mb-4">
        ...
        <div
          class="bg-teal-50 text-teal-700 text-xs font-semibold px-2.5 py-1 rounded-full border border-teal-100"
        >
    ...
    <div
      v-show="isExpanded"
      class="bg-slate-50 border-t border-slate-100 transition-all duration-300"
    >
      ...
      <div v-else-if="variantes.length === 0" class="p-6 text-center">
        ...
        <button
          @click="$emit('create-variante')"
          class="mt-2 text-xs font-bold text-teal-600 hover:text-teal-700"
        >
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full text-left text-xs">
          <thead>
            <tr class="bg-slate-100/50 text-slate-500 font-bold border-b border-slate-200">
              <th class="px-4 py-3">Variante</th>
              <th class="px-4 py-3 text-right">Valor</th>
              <th class="px-4 py-3 text-right">Seña</th>
              <th class="px-4 py-3">Régimen</th>
              <th class="px-4 py-3">Butaca</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100">
            ...
            >
              <td class="px-4 py-3 font-semibold text-slate-700">{{ variante.nombreVariante }}</td>
              ...
              <td class="px-4 py-3 text-slate-600">{{ getRegimenLabel(variante.regimen) }}</td>
              <td class="px-4 py-3 text-slate-600">{{ variante.tipoDeButaca }}</td>
            </tr>
          </tbody>
        </table>
      </div>
