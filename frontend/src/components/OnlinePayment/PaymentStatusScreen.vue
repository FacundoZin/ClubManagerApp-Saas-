<template>
  <div class="max-w-xl mx-auto mt-12 animate-fadeIn">
    <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden text-center pb-12">
      <!-- Icon Header -->
      <div :class="[
        ...
        <div class="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-sm mb-4 relative">
          <!-- Éxito -->
          <svg v-if="isApproved" xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-blue-500" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <!-- Rechazado -->
          <svg v-else-if="status === 'rejected'" xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-red-500"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
          </svg>
          <!-- Procesando (Spinner) -->
          <div v-else class="relative w-12 h-12">
            <div class="absolute inset-0 border-4 border-indigo-100 border-t-indigo-600 rounded-full animate-spin">
            </div>
          </div>
        </div>
