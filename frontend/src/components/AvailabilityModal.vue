...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
      <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm border border-slate-200">
        ...
        <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
             ...
             <div v-else>
                 <div class="mx-auto flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full sm:mx-0 sm:h-12 sm:w-12 mb-4 mx-auto"
                    :class="result?.exit ? 'bg-emerald-100' : 'bg-red-100'">
                    
                    <svg v-if="result?.exit" class="h-8 w-8 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                 ...
                 <div class="text-center">
                    <h3 class="text-lg font-bold text-slate-900 mb-2">
                        {{ result?.exit ? 'Disponible' : 'No Disponible' }}
                    </h3>
                    <p class="text-sm text-slate-600">
                        {{ result?.data || result?.errormessage || 'Información de disponibilidad.' }}
                    </p>
                    <p v-if="result?.exit" class="text-xs text-emerald-600 mt-2 font-medium">
