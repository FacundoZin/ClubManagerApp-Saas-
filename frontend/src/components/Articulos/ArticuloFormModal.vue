    ...
    <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
      <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-200">
        ...
        <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 border-b border-slate-100">
          <h3 class="text-lg font-semibold leading-6 text-slate-900">Nuevo Artículo de Ortopedia</h3>
        </div>
        ...
        <form @submit.prevent="handleSubmit">
          <div class="px-4 py-5 sm:p-6 space-y-4">
             ...
             <div>
                ...
                <input type="text" v-model="form.nombre" required placeholder="Ej: Silla de ruedas"
             ...
             <div>
                <label class="block text-sm font-medium text-slate-700">Precio de Alquiler Mensual</label>
                <div class="relative mt-1 rounded-md shadow-sm">
                    <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        <span class="text-slate-500 sm:text-sm">$</span>
                    </div>
                    <input type="number" v-model="form.precioAlquiler" min="1" required
