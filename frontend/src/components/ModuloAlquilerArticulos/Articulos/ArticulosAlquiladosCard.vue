...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
    ...
    <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
      ...
      <button @click="emit('add-item')" class="text-sm font-medium text-teal-600 hover:text-teal-800 flex items-center">
    ...
    <div class="sm:hidden divide-y divide-slate-100">
      <div v-for="item in items" :key="item.id" class="p-4 space-y-3">
        ...
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            <span class="text-slate-400 block text-xs uppercase font-bold tracking-wider">Precio Unit.</span>
            <span class="text-slate-700 font-medium">${{ item.precioAlquiler.toLocaleString() }}</span>
          </div>
          <div class="text-right">
            <span class="text-slate-400 block text-xs uppercase font-bold tracking-wider">Subtotal</span>
      ...
      <table class="min-w-full divide-y divide-slate-200">
        ...
        <tbody class="divide-y divide-slate-200 bg-white">
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
              <div class="flex items-center border border-slate-200 rounded-lg overflow-hidden bg-slate-50 w-fit">
                <button @click="handleUpdateQuantity(item, item.cantidad - 1)"
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
              <button @click="handleRemoveItem(item.id)" class="text-rose-600 hover:text-rose-900 font-medium">
                Eliminar
              </button>
            </td>
          </tr>
